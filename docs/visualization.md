...
### Generation Gallery (Paper Figure)

Produce a paper-quality grid figure comparing molecules generated by each MOSAIC
tokenizer variant side-by-side with reference molecules of increasing size. Each
row shows one reference molecule sampled from the corresponding dataset (MOSES
test set or COCONUT) and the closest-sized generation from each of 6 models
(SENT, HSENT_MC, HSENT_SC, HDT_MC, HDT_SC, HDTC).

Reference molecules are automatically sampled to span the atom-count range of
the dataset (e.g., 12-26 for MOSES, 25-100 for COCONUT). The script loads each
model sequentially, generates a batch of molecules, filters for validity, and
greedily selects the closest match to each reference's atom count. Failed
generations show a red-tinted placeholder.

```bash
# Quick test (2 rows, 16 samples per model)
python scripts/visualization/generation_gallery.py --dataset moses --num-samples 2 --num-generate 16 --no-show

# Full paper figure (MOSES)
python scripts/visualization/generation_gallery.py --dataset moses --num-samples 5 --dpi 300 --no-show

# Full paper figure (COCONUT)
python scripts/visualization/generation_gallery.py --dataset coconut --num-samples 5 --dpi 300 --no-show

# Fewer rows, more candidates for better size matching
python scripts/visualization/generation_gallery.py --dataset moses --num-samples 3 --num-generate 128
```

| Argument | Default | Description |
|----------|---------|-------------|
| `--dataset` | `moses` | `moses` or `coconut` (selects checkpoint set) |
| `--num-samples` | `5` | Number of rows (reference molecules) |
| `--num-generate` | `64` | Molecules generated per model for selection |
| `--output-dir` | `./figures` | Output directory |
| `--dpi` | `200` | Output DPI |
| `--seed` | `42` | Random seed |
| `--no-show` | flag | Suppress `plt.show()` |
